...
def _export_model_files_to_ai_pack(
  ...
  """Exports the model files to the AI pack directory format."""
...
def _write_ai_pack_manifest(ai_pack_dir: pathlib.Path):
  """Writes the AI pack manifest file."""
...
def _build_targeting_config(compiled_targets: list[types.Target]) -> str:
  """Builds the device targeting config for the given compiled targets."""
  device_groups = []
...
def _target_to_ai_pack_info(target: types.Target) -> str | None:
  """Returns the device targeting info for the given target."""
  if isinstance(target, qnn_target.Target):
    ...
    # TODO(weiyiw): Support other targets.
    print('unsupported target ', target)
    return None


# TODO(weiyiw): Auto-generate this function from CSVs.
...
def _write_targeting_config(
  ...
  """Writes the device targeting config for the given compiled models."""
  compiled_targets = [
...
def ai_pack_export(
  ...
  os.makedirs(ai_pack_dir, exist_ok=True)
