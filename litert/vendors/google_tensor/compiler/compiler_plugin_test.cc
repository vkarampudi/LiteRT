...
bool IsOpSupportedByFilters(const ::litert::Op& op,
                            const ::third_party::odml::litert::litert::vendors::
...
TEST(TestCallGoogleTensorPlugin, PartitionWithOpFiltersRunOnCpu) {
  ...
  out << "filter_behavior: MATCHES_RUN_ON_CPU\n";
  out << "filters {\n";
  out << "  op_name_pattern: \".*\"\n";  // Block all operations
  out << "}\n";
  out.close();

  google_tensor_options.SetOpFiltersProtoTextFile(temp_file);
...
TEST(TestGoogleTensorPlugin, IsOpSupportedByFilters) {
  auto model = testing::LoadTestFileModel("simple_multi_op.tflite");
  LITERT_ASSERT_OK_AND_ASSIGN(auto subgraph, model.Subgraph(0));
  ...
  // 1. Empty filter list -> Supported
  EXPECT_TRUE(::google_tensor::IsOpSupportedByFilters(op, filters));

  // 2. MATCHES_RUN_ON_CPU (Blocklist) - Match
  filters.set_filter_behavior(OpFilters::MATCHES_RUN_ON_CPU);
  auto* filter1 = filters.add_filters();
  filter1->set_op_name_pattern(std::string(output_name));
  EXPECT_FALSE(::google_tensor::IsOpSupportedByFilters(op, filters));

  // 3. MATCHES_RUN_ON_CPU (Blocklist) - No Match
  filter1->set_op_name_pattern("some_unmatched_name");
  EXPECT_TRUE(::google_tensor::IsOpSupportedByFilters(op, filters));

  // 4. MATCHES_RUN_ON_TPU (Allowlist) - No Match
  filters.set_filter_behavior(OpFilters::MATCHES_RUN_ON_TPU);
  EXPECT_FALSE(::google_tensor::IsOpSupportedByFilters(op, filters));

  // 5. MATCHES_RUN_ON_TPU (Allowlist) - Match
  filter1->set_op_name_pattern(std::string(output_name));
  EXPECT_TRUE(::google_tensor::IsOpSupportedByFilters(op, filters));

  // 6. Empty op_name_pattern (should be ignored)
