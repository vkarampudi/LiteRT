...
// where by that we mean not just no longer used at head, but also no longer
// used by already built app binaries -- verifying this may require adding
// logging.  Entries that are really not longer used at all, even by old code,
// can be replaced by a placeholder of the same signature, e.g. a function
// pointer that is set to a no-op function, but should not be deleted, to avoid
// affecting the offsets of entries later in the table.  Any new methods should
// only be added at the END of the table (unless reusing a slot occupied by
// a placeholder).
